...
[[maybe_unused]] static void encode_gl_clear(u32* data, size_t& used, size_t max, u8 r, u8 g, u8 b) {
    VERIFY(used + 9 <= max);
...
void GPU3DDevice::unregister_scanout_framebuffer(ResourceID)
{
}
...
ErrorOr<void> GPU3DDevice::ioctl(OpenFileDescription&, unsigned request, Userspace<void*> arg) {
    ...
    case VIRGL_IOCTL_GET_DISPLAY_INFO: {
        ...
        display_info.drawtarget_id = m_drawtarget_resource_id.value();
        return copy_to_user(static_ptr_cast<VirGLDisplayInfo*>(arg), &display_info);
    }
